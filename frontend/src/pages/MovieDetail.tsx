...
export default function MovieDetail() {
  ...
  const fetchMovie = async () => {
    try {
      const response = await axios.get(`https://movie-tracker-backend-acc8.onrender.com/movies/${id}`, {
  ...
  const fetchCast = async (tmdbId: number) => {
    try {
      const response = await axios.get(`https://movie-tracker-backend-acc8.onrender.com/movies/cast/${tmdbId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
